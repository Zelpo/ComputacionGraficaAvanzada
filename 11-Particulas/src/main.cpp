...
// Se crea el identificador para generar las velocidades y el tiempo de inicio de las partículas
...
void initParticleBuffers() {
	...
	// El tamaño del buffer = número de partículas * 3 (x, y, z) * tamaño de un flotante
	...
	// Se llama el buffer con las velocidades, las velocidades son aleatorias pero con un patrón en coordenadas esféricas
	...
	float time = 0.0f;
	float rate = 0.00075f;
	// Cada partícula se va a llenar en un determinado tiempo
	for (unsigned int i = 0; i < nParticles; i++) {
		data[i] = time;
		time += rate;
	}
	...
	while (psi) {
		...
		for(std::map<float, std::pair<std::string, glm::vec3> >::reverse_iterator it = blendingSorted.rbegin(); it != blendingSorted.rend(); it++){
			if(it->second.first.compare("aircraft") == 0){
				...
				glBindVertexArray(VAOParticles);
				// Como primitiva del sistema de particulas son puntos. Cuantos puntos = nParticles
			...
			else if (it->second.first.compare("fountain2") == 0) {
				/**********
				 * Init Render particles systems
				 */
				glm::mat4 modelMatrixParticlesFountain = glm::mat4(1.0);
